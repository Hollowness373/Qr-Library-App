...
if (!empty($_POST['QrScanned']))  {
    ...
    if ($CheckDataName == $QrScanned){
         $db_query = "UPDATE timeinandoutdb SET TimeIn = '$Time' WHERE ID = $CheckID";
    ...
    if ($V) {
        $Message = "Success";
          ...
          $db_querys = "INSERT into timeinandoutdb (FullName, Course, TimeIn) values('" .$QrScanned. "','" .$Course. "','" .$Time. "')";
          $X = $conn->query($db_querys);
